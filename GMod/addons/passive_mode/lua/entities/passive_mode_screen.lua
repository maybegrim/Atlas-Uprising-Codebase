...
ENT.Category = "[AU] Passive Mode"
ENT.Spawnable = true
 
ENT.PrintName		= "Passive Mode Screen"
ENT.Author			= "Buckell"
...
if SERVER then
    ...
    function ENT:Use(activator, caller)
        ...
        caller:SetPassiveMode(not caller:InPassiveMode())
    end
end

if CLIENT then
    surface.CreateFont("PassiveMode.Screen", {
        font = "Courier New",
        size = 40,
        antialias = true,
        weight = 700
    ...
    function ENT:Draw()
        ...
        cam.Start3D2D(self:LocalToWorld(Vector(-23.7, -47.5, 1.6)), self:LocalToWorldAngles(Angle(0, 90, 0)), 0.1)
            draw.DrawText("PASSIVE", "PassiveMode.Screen", 1900 / 4, 200, Color(0, 200, 0, alpha), TEXT_ALIGN_CENTER)
            draw.DrawText("MODE", "PassiveMode.Screen", 1900 / 4, 240, Color(0, 200, 0, alpha), TEXT_ALIGN_CENTER)
        cam.End3D2D()
    end
end