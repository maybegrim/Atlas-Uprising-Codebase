...
// Add to
concommand.Add("perm_sweps_add", function(ply, cmd, args, argStr)
    print("perm_sweps_add command called")
    if !IsValid(ply) or ply:IsSuperAdmin() then
        print("Player is valid or is SuperAdmin")
        if args[1] and args[2] then
            local target = args[1]  -- SteamID without quotes
            print("Target SteamID:", target)
            if string.Left(target, 5) == "STEAM" then
                -- Remove the SteamID from the args and continue with weapons
                table.remove(args, 1)
                local weps = forceswepcheck and getValidSWEPS(args) or args
                print("Weapons to add:", weps)
                local oldweps = util.GetPData(target, "PermSweps", false)
                if oldweps then
                    print("Old weapons found:", oldweps)
                    weps = tableAdd(weps, util.JSONToTable(oldweps))
                end
                if #weps >= 1 then
                    util.SetPData(target, "PermSweps", util.TableToJSON(weps))
                    print("New weapons list saved:", weps)
                    local real = player.GetBySteamID(target)
                    if real then
                        PermSweps[real] = weps
                        print("Updated PermSweps for player:", real)
                    end
                end
            end
        end
...
concommand.Add("perm_sweps_remove", function(ply, cmd, args, argStr)
    print("perm_sweps_remove command called")
    if !IsValid(ply) or ply:IsSuperAdmin() then
        print("Player is valid or is SuperAdmin")
        if args[1] and args[2] then
            local target = args[1]  -- SteamID without quotes
            ...
            if string.Left(target, 5) == "STEAM" then
                table.remove(args, 1)
                local weps = forceswepcheck and getValidSWEPS(args) or args
                print("Weapons to remove:", weps)
                local oldweps = util.GetPData(target, "PermSweps", false)
                if !oldweps then 
                    print("No old weapons found")
                    return 
                end
                if #weps >= 1 then
                    local newweps = {}
                    ...
                    print("New weapons list saved:", newweps)
                    local real = player.GetBySteamID(target)
                    if real then
