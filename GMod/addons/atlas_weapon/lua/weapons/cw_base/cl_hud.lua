	...
	if not disableCustomHUD then
		...
		if not customHUD and GetConVarNumber("cw_customhud_ammo") <= 0 then
			...
			elseif self.FireModeDisplayPos == "middle" then
				draw.ShadowText(self.FireModeDisplay, cwhud16, x * 0.5, y - 50, White, Black, 1, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
				
				local grenades = self.Owner:GetAmmoCount("Frag Grenades")
				
				-- only display the frag ammo count if we have any grenades in reserve
				if grenades > 0 then
					draw.ShadowText(self:getFragText(grenades), cwhud16, x * 0.5, y - 130, self.HUD_ReserveTextColor, self.HUDColors.black, 1, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
				end
				...
				if self.BulletDisplay and self.BulletDisplay > 0 then
					...
					for i = 1, self.BulletDisplay do
						surface.DrawTexturedRect(x * 0.5 - 20 * i + self.BulletDisplay * 10 + 1, y - 71, 16, 16)
					end
					
					surface.SetDrawColor(255, 255, 255, 255)
					
					for i = 1, self.BulletDisplay do
						surface.DrawTexturedRect(x * 0.5 - 20 * i + self.BulletDisplay * 10, y - 72, 16, 16)
					end
				end
...
function SWEP:drawTabDisplay()
	...
	surface.SetDrawColor(44, 44, 79, 250)
	--surface.SetTexture(gradient)
	surface.DrawTexturedRectRotated(155, y - 974 - baseHeight * 0.5, 320, baseHeight, 360)
			
	for k, v in ipairs(CustomizableWeaponry.interactionMenu.tabs) do
		local clr
		
		if self.CustomizationTab == v.id then
			clr = self.HUDColors.red
		else
			clr = self.HUDColors.white
		end
		
		draw.ShadowText(self:getKeyBind(v.switchToKey) .. " - " .. v.text, cwhud24, 10, y - 1000 - (k - 1) * 23, clr, self.HUDColors.black, 1, TEXT_ALIGN_LEFT, TEXT_ALIGN_RIGHT)
	end
end
