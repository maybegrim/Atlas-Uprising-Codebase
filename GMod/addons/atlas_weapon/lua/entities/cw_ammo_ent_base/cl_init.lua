...
local white, black, red, green, yellow = Color(255, 255, 255, 255), Color(0, 0, 0, 255), Color(255,56,56), Color(130, 205, 96), Color(255, 192, 56)
...
function ENT:Draw()
	...
	cam.Start3D2D(self:GetPos() + self.upOffset, eyeAng, 0.05)
		...
		surface.SetDrawColor(255, 56, 56, a)
		...
		if not self.CaliberSpecific then
			...
			draw.ShadowText(self.dt.ammoCharge .. (self.dt.ammoCharge == 1 and "x resupply left" or "x resupplies left"), ammoText, 0, self.vertFontSize + self.halfVertFontSize, self.dt.ammoCharge < 3 and red or self.dt.ammoCharge < 5 and yellow or white, black, 2, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
			
			local wep = ply:GetActiveWeapon()
			
			if IsValid(wep) and wep.CW20Weapon then
				draw.ShadowText("+" .. wep.Primary.ClipSize_Orig .. " rounds per resupply", ammoText, 0, self.vertFontSize * 2 + self.halfVertFontSize, green, black, 2, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
			else
				draw.ShadowText("CANNOT RESUPPLY", ammoText, 0, self.vertFontSize * 2 + self.halfVertFontSize, red, black, 2, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
			end
		else
